...
async def set_not_afk(event):
    ...
    if (("afk" not in current_message) or ("#afk" not in current_message)) and (
        ...
        shite = await event.client.send_message(
            event.chat_id,
            "♰︙** تم تعطيـل امر وضع الطيران والـرجوع الى الوضع الطبيعي**",
        )
        AFK_.USERAFK_ON = {}
        AFK_.afk_time = None
...
async def on_afk(event):  # sourcery no-metrics
    ...
    if AFK_.USERAFK_ON and not (await event.get_sender()).bot:
        ...
        if AFK_.afk_type == "media":
            if AFK_.reason:
                message_to_reply = f"**♰︙انا الان في وضعيه عدم الاتصال أنَني ربما نائم أو مسافر أو مشغول ✈** \n**♰︙تم تفعيل وضع الطيران منذ 🕐 :** `{endtime}`\n ♰︙السـبب : {AFK_.reason}"
            else:
                message_to_reply = f"**♰︙انا الان في وضعيه عدم الاتصال أنَني ربما نائم أو مسافر أو مشغول ✈** \n**♰︙تم تفعيل وضع الطيران منذ 🕐 :** `{endtime}`"
            if event.chat_id:
...
async def _(event):
    ...
    if not AFK_.USERAFK_ON:
        ...
        if AFK_.reason:
            await edit_delete(
                event,
                f"♰︙انا الان في وضعيه عدم الاتصال أنَني ربما نائم أو مسافر أو مشغول يرجـى المراسلة لاحقـا بسـبب  {AFK_.reason} ️",
                5,
            )
        else:
            await edit_delete(
                event,
                f"**♰︙انا الان في وضعيه عدم الاتصال أنَني ربما نائم أو مسافر أو مشغول يرجـى المراسلة لاحقـا ✈ ️**",
                5,
            )
        if BOTLOG:
            if AFK_.reason:
...
async def _(event):
    ...
    if media_t == "Sticker" or not media_t:
        return await edit_or_reply(
            event, "♰︙امـر وضع الطيران : المرجـو قم بالـرد علـى الصورة بالامـر "
        )
    if not BOTLOG:
        return await edit_or_reply(
