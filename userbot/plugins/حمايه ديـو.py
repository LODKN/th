...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"""هها خييہَ {mention} ؟  ♰\nلتعبہَ نفسكہَ انا ماموجود ♰\nدز ࢪسالتكہَ ولتعبنيہَ  ♰\nعندكہَ 𝟕 تحذيراتہَ اذا تدز بعد ال𝟕\nتحذيراتہَ احظركہَ كبلہَ ♰\nيلا سيوَ ♰\nﻣطۅࢪين … @nnnuu\n♰ {warns}/{totalwarns} ♰"""
    else:
        USER_BOT_NO_WARN = f"""هها خييہَ {mention} ؟  ♰\nلتعبہَ نفسكہَ انا ماموجود ♰\nدز ࢪسالتكہَ ولتعبنيہَ  ♰\nعندكہَ 𝟕 تحذيراتہَ اذا تدز بعد ال𝟕\nتحذيراتہَ احظركہَ كبلہَ ♰\nيلا سيوَ ♰\nﻣطۅࢪين … @nnnuu\n♰ {warns}/{totalwarns} ♰"""
...
async def on_plug_in_callback_query_handler(event):
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق ماتركـس
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
async def approve_p_m(event):  # sourcery no-metrics
    ...
    if gvarstatus("pmpermit") is None:
        return await edit_delete(
            event,
            f"♰︙يـجب تفعيـل امـر الحـماية اولا بأرسـال `{cmdhd}تفع` لـيشتغل هذا الأمـر",
        )
    if event.is_private:
...
async def block_p_m(event):
    ...
    if gvarstatus("pmpermit") is None:
        return await edit_delete(
            event,
            f"♰︙يـجب تفعيـل امـر الحـماية اولا بأرسـال `{cmdhd}تفع` لـيشتغل هذا الأمـ ",
        )  # ترجمه وكتابة فريق ماتركـس
    if event.is_private:
        user = await event.get_chat()
